...
- Move back on the main branch: `git checkout main`
...
- Check the difference between the *main* and the *feature/merge-example* branches: `git log main..feature/merge-example`
- You can see that the *main* branch is ahead by two commits
- Integrate these commits into your branch by using `git rebase main`
- Check the difference between branches again: `git log main..feature/merge-example`

## Git Rebase and Fast Forward Merge

One thing that happens often is that people will merge the integration branch (main in our case) back into their branch to grab the latest changes. This will create a new commit representing the merge of the two branches. Then they will merge the feature branch into the integration branch, essentially duplicating some commits on the main branch.  
...
Then we can safely Fast Forward Merge our changes onto the integration branch. This will replay our commits onto the destination branch, essentially making it look like we worked directly on the *main* branch and providing a clean history.

### Example Rebase and FF Merge

- Make sure you're on the main branch: `git checkout main`  
- Create a new commit: `Write-Output "Commit 1" | Out-File -FilePath ./RebaseFF1.txt && git add . && git commit -m "Another commit on main branch"`
- Move back to your *feature/merge-example* branch: `git checkout feature/merge-example`
...
- Integrate the commits from *main* into your branch by using `git rebase main`
- Move back to *main*: `git checkout main`  
- Replay your commits on the main branch with: `git merge --ff-only feature/merge-example`
...
- Move back to *main* and create another branch called *feature/branch2*:
  - `git checkout main`
